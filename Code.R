...
setwd("C:/Personal Data/Essex/Term 1/MA334 - Data analysis and statistics with R/Project/Folder of files to use and study for the assessed individual project-20230415")
...
#Proj_data <- Proj_data%>%filter(dominantLandClass=="10e")

# allocated 7 predictors to form the trial eco_stat

all <- c(2:12)
eco_selected <- sample(all,size=7, replace = FALSE)
eco_selected <- c(2,3,4,5,6,8,9)   # a particular troublesome case
...
table <- data.frame()
i <- 1
while (i <= length(eco_selected)) {
  j <- eco_selected[i]
  table <- rbind(table,
                 c(eco_names[j-1],
                   round(mean(Proj_data_MA334[,j],na.rm = TRUE),digits = 2),
                   round(sd(Proj_data_MA334[,j],na.rm = TRUE),digits = 2),
                   round(skewness(Proj_data_MA334[,j],na.rm = TRUE),digits = 2)
                 ))
  i <- i + 1
}
...
par(mar=c(5,5,4,2)+0.1) # increase the size of the plot margins
plot.window(c(1, 2), c(1, 2), asp = 1)
plot(cont_vars$Northing~cont_vars$Easting, xlab = "Easting", ylab = "Northing")
...
#The resulting PCA plot shows the spread of the biodiversity data in two dimensions and labels the locations with their dominant land class.
...
eco_selected <- c(2,3,4,5,6,8,9)#c(12,2,5,6,3,7,4) # an example Bees !
eco_selected
eco_difference <- Proj_data_MA334_Y00[,eco_selected ]-Proj_data_MA334_Y70[,eco_selected ]# general differences between the two periods 
eco_selected
pr.out=prcomp(na.omit(eco_difference)) # Principal Components 
